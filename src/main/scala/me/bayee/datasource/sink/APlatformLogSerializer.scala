...
/**
  ...
  */
class APlatformLogSerializer(context: Context, out: OutputStream) extends AbstractSerializer(context, out) {
  override def schemaPath: String = "schema/4APlatformLog.avsc"
  ...
  override def write(event: Event): Unit = try {
    ...
    if(auditLog.getLogclass == 20) {
      ...
      record.put("recname", new String(auditLog.getRecname.toByteArray))
      record.put("filluuid", new String(auditLog.getFilluuid.toByteArray))
      record.put("reserved3", new String(auditLog.getReserved3.toByteArray))
...
class APlatformLogSerializerBuilder extends EventSerializer.Builder {
  override def build(context: Context, outputStream: OutputStream): EventSerializer = new APlatformLogSerializer(context, outputStream)
}