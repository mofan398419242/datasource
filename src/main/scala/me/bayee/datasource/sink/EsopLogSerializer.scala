...
/**
  ...
  */
class EsopLogSerializer(context: Context, out: OutputStream) extends AbstractSerializer(context, out) {
  override def schemaPath: String = "schema/EsopLog.avsc"
  ...
  override def write(event: Event): Unit = try {
    ...
    if(auditLog.getAuditLogtypeid == -18 && auditLog.getLogclass == 6) {
...
class EsopLogSerializerBuilder extends EventSerializer.Builder {
  override def build(context: Context, outputStream: OutputStream): EventSerializer = new EsopLogSerializer(context, outputStream)
}