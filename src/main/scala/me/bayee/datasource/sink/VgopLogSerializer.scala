...
/**
  ...
  */
class VgopLogSerializer(context: Context, out: OutputStream) extends AbstractSerializer(context, out) {
  override def schemaPath: String = "schema/VgopLog.avsc"
  ...
  override def write(event: Event): Unit = try {
    ...
    if(auditLog.getAuditLogtypeid == -25 && auditLog.getLogclass == 6) {
      ...
      record.put("account_status", auditLog.getAccountStatus)
      ...
      record.put("workinfo", new String(auditLog.getWorkinfo.toByteArray))
...
class VgopLogSerializerBuilder extends EventSerializer.Builder {
  override def build(context: Context, outputStream: OutputStream): EventSerializer = new VgopLogSerializer(context, outputStream)
}