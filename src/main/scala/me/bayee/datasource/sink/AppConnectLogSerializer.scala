...
/**
  ...
  */
class AppConnectLogSerializer(context: Context, out: OutputStream) extends AbstractSerializer(context, out) {
  override def schemaPath: String = "schema/AppConnectLog.avsc"
  ...
  override def write(event: Event): Unit = try {
    ...
    if(auditLog.getAuditLogtypeid == -49 && auditLog.getLogclass == 6) {
...
class AppConnectLogSerializerBuilder extends EventSerializer.Builder {
  override def build(context: Context, outputStream: OutputStream): EventSerializer = new AppConnectLogSerializer(context, outputStream)
}