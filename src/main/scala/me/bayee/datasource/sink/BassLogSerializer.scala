...
/**
  ...
  */
class BassLogSerializer(context: Context, out: OutputStream) extends AbstractSerializer(context, out) {
  override def schemaPath: String = "schema/BassLog.avsc"
  ...
  override def write(event: Event): Unit = try {
    ...
    if(auditLog.getAuditLogtypeid == -5 && auditLog.getLogclass == 6) {
...
class BassLogSerializerBuilder extends EventSerializer.Builder {
  override def build(context: Context, outputStream: OutputStream): EventSerializer = new BassLogSerializer(context, outputStream)
}