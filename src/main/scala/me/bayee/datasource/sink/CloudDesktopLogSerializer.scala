...
/**
  ...
  */
class CloudDesktopLogSerializer(context: Context, out: OutputStream) extends AbstractSerializer(context, out) {
  override def schemaPath: String = "schema/CloudDesktopLog.avsc"
  ...
  override def write(event: Event): Unit = try {
    ...
    if(auditLog.getAuditLogtypeid == -38 && auditLog.getLogclass == 6) {
      ...
      record.put("reserved1", new String(auditLog.getReserved1.toByteArray))
...
class CloudDesktopLogSerializerBuilder extends EventSerializer.Builder {
  override def build(context: Context, outputStream: OutputStream): EventSerializer = new CloudDesktopLogSerializer(context, outputStream)
}
