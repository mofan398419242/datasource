...
/**
  ...
  */
class BomcLogSerializer(context: Context, out: OutputStream) extends AbstractSerializer(context, out) {
  override def schemaPath: String = "schema/BomcLog.avsc"
  ...
  override def write(event: Event): Unit = try {
    ...
    if(auditLog.getAuditLogtypeid == -16 && auditLog.getLogclass == 6) {
...
class BomcLogSerializerBuilder extends EventSerializer.Builder {
  override def build(context: Context, outputStream: OutputStream): EventSerializer = new BomcLogSerializer(context, outputStream)
}