...
/**
  ...
  */
class GoldOperationLogSerializer(context: Context, out: OutputStream) extends AbstractSerializer(context, out) {
  override def schemaPath: String = "schema/GoldOperationLog.avsc"
  ...
  override def write(event: Event): Unit = try {
    ...
    if (auditLog.getLogclass == 5) {
      ...
      record.put("memo", new String(auditLog.getMemo.toByteArray))
...
class GoldOperationLogSerializerBuilder extends EventSerializer.Builder {
  override def build(context: Context, outputStream: OutputStream): EventSerializer = new GoldOperationLogSerializer(context, outputStream)
}